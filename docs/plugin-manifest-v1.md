...
Protocol bridge MVP skeleton 只稳定 manifest 声明、capability dependency 和安装预检展示。完整协议互换执行链仍属于 future host integration；插件不能只靠声明 `protocolBridges` 就接管 OpenAI、Gemini 或 Claude 协议转换。
