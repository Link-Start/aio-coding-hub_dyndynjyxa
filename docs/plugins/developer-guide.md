...
6. 使用 `pnpm --filter create-aio-plugin cli replay --explain` 在本地 fixture 上验证 hook 行为。
...
9. 使用 `pnpm --filter create-aio-plugin cli pack` 打包为 `.aio-plugin`。
10. 发布前运行 `pnpm --filter create-aio-plugin cli publish-check ./acme.redactor` 生成 release metadata。
